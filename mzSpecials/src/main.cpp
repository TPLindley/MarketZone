...
#include <memory>
#include "json.hpp"
#include "version.h"   // auto-generated by cmake/UpdateVersion.cmake
#include "httplib.h"
...
static std::string library_file_path()
{
    std::string dir = std::string(g_get_user_data_dir()) + "/mzSpecials";
    std::filesystem::create_directories(dir);
...
static void save_library(const std::vector<Special>& library)
{
    log("INFO", "Saving " + std::to_string(library.size()) + " library entries to " + library_file_path());
    json arr = json::array();
    for (const auto& s : library)
        arr.push_back({{"text", s.text}, {"color", s.color_hex}});
    std::ofstream f(library_file_path());
    f << arr.dump(2);
}

static std::vector<Special> load_library()
{
    std::string path = library_file_path();
    std::ifstream f(path);
    if (!f.is_open()) {
        log("INFO", "No library file at " + path + ", starting empty");
        return {};
    }
    try {
        json arr = json::parse(f);
        ...
        log("INFO", "Loaded " + std::to_string(v.size()) + " library entries from " + path);
        return v;
    } catch (...) {
        log("WARN", "Failed to parse library file at " + path + ", starting empty");
        return {};
    }
}
...
static std::vector<Special> g_library;
...
// -----------------------------------------------------------------------
// Merge specials into the library — adds entries whose text is not already present.
// Must be called with g_mutex held. Returns the number of new entries added.
static int merge_into_library(const std::vector<Special>& specials)
{
    int added = 0;
    for (const auto& s : specials) {
        auto it = std::find_if(g_library.begin(), g_library.end(),
            [&](const Special& lib) { return lib.text == s.text; });
        if (it == g_library.end()) {
...
static void set_json(httplib::Response& res, json obj)
{
    ...
    obj["uptime_seconds"] = (long long)std::chrono::duration_cast<std::chrono::seconds>(
...
static void run_api_server()
{
    ...
    svr.Post("/specials", [](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            int added = 0;
            std::vector<Special> library_copy;
            {
                std::lock_guard<std::mutex> lk(g_mutex);
                g_specials = newlist;
                added = merge_into_library(newlist);
                if (added > 0)
                    library_copy = g_library;
            }
            save_specials(newlist);
            if (added > 0)
                save_library(library_copy);
            schedule_rebuild();
            log("HTTP", "POST /specials -> accepted " + std::to_string(newlist.size()) + " items, " + std::to_string(added) + " new library entries");
            set_json(res, {{"status", "ok"}, {"count", (int)newlist.size()}, {"library_added", added}});
        } catch (const std::exception& e) {
    ...
    // GET /library — return full library of all specials ever used
    svr.Get("/library", [](const httplib::Request& req, httplib::Response& res) {
        log("HTTP", "GET /library from " + req.remote_addr);
        json arr = json::array();
        size_t count = 0;
        {
            std::lock_guard<std::mutex> lk(g_mutex);
            for (const auto& s : g_library)
                arr.push_back({{"text", s.text}, {"color", s.color_hex}});
            count = g_library.size();
        }
        set_json(res, {{"library", arr}});
        log("HTTP", "GET /library -> returned " + std::to_string(count) + " entries");
    });

    // POST /library/sync — clear library and replace with current active specials
    svr.Post("/library/sync", [](const httplib::Request& req, httplib::Response& res) {
        log("HTTP", "POST /library/sync from " + req.remote_addr);
        std::vector<Special> synced;
        {
            std::lock_guard<std::mutex> lk(g_mutex);
            g_library = g_specials;
            synced = g_library;
        }
        save_library(synced);
        log("HTTP", "POST /library/sync -> synced " + std::to_string(synced.size()) + " entries");
        set_json(res, {{"status", "ok"}, {"count", (int)synced.size()}});
    });

    // GET /status — return version and uptime
    svr.Get("/status", [](const httplib::Request& req, httplib::Response& res) {
        long long uptime = (long long)std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::steady_clock::now() - g_start_time).count();
        log("HTTP", "GET /status from " + req.remote_addr
            + " -> uptime=" + std::to_string(uptime) + "s");
        set_json(res, json::object());
    });
